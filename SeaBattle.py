    ...
    def greet(self):
        ...
        try:
            board_size[1], board_size[0], *a = (
                map(int, input('\nChoose board size(width & height, from 3 to 10, default 6x6): ').split()))
        except ValueError:
            pass
        n = input('Number of three deck ships (default 1): ')
        if not n.isdigit() or int(n) < 0 or int(n) > 3:
            n = 1
        for i in range(int(n)):
            ordered_ships.append(3)
        n = input('Number of two deck ships (default 2): ')
        if not n.isdigit() or int(n) < 0 or int(n) > 5:
            n = 2
        for i in range(int(n)):
            ordered_ships.append(2)
        n = input('Number of one deck ships (default 4): ')
